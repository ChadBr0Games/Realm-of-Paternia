...
class adap_comp extends SpriteAnimationComponent with HasGameRef<Realm_of_Patternia>{
  final String _name;
  adap_comp( { position, required String name, super.key}): _name = name, super (position: position, );
  ...
  FutureOr<void> onLoad() {
    ...
    animation = SpriteAnimation.fromFrameData(game.images.fromCache('Pattern_Components/Items/Weather_scroll.png'), 
  ...
  void collecting(){
    ...
    if(!collected){
      ...
      if(adap_comps_num==0){AdapterSW.start();}
      game.overlays.remove('DialogueOverlay');
      AdapterComps[int.parse(_name)]=true;
      adap_comps_num += 1;
      animation= _spriteAniamtion(15, 7);
      collected = true;
      String part_name = adap_strcut[int.parse(_name)];
