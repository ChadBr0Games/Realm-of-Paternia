...
class fac_comp extends SpriteAnimationComponent with HasGameRef<Realm_of_Patternia>{
  final String _name;
  fac_comp( { position, required String name, super.key}): _name = name, super (position: position, );
  ...
  FutureOr<void> onLoad() {
    ...
    animation = SpriteAnimation.fromFrameData(game.images.fromCache('Pattern_Components/Items/Gear.png'), 
  ...
  void collecting(){
    ...
    if(!collected){
      ...
      if(fac_comps_num==0){FactorySW.start();}
      game.overlays.remove('DialogueOverlay');
      factoryComps[int.parse(_name)]=true;
      fac_comps_num += 1;
      animation= _spriteAniamtion(1, 7);
      collected = true;
      String part_name = fact_strcut[int.parse(_name)];
