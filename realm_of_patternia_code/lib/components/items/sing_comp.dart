...
class sing_comp extends SpriteAnimationComponent with HasGameRef<Realm_of_Patternia>{
  final String _name;
  sing_comp( { position, required String name, super.key}): _name = name, super (position: position, );
  ...
  FutureOr<void> onLoad() {
    ...
    animation = SpriteAnimation.fromFrameData(game.images.fromCache('Pattern_Components/Items/Tax_coin.png'), 
  ...
  void collecting(){
    ...
    if(!collected){
      ...
      if(sing_comps_num==0){SingletonSW.start();}
      game.overlays.remove('DialogueOverlay');
      SingletonComps[int.parse(_name)]=true;
      sing_comps_num +=1;
      animation= _spriteAniamtion(11, 7);
      collected = true;
      String part_name = sing_strcut[int.parse(_name)];
